        ...
        string currentContent)
    {
        try
        {
            ...
            foreach (Report previousReport in previousReports.Where(report => report.FileId != currentFileId))
            {
                ...
                {
                    logger.LogWarning(ex, "Ошибка доступа к файлу отчета {ReportId}", previousReport.ReportId);
                }
                catch (UnauthorizedAccessException ex)
                {
                    logger.LogWarning(ex, "Нет доступа к файлу отчета {ReportId}", previousReport.ReportId);
                }
            }
        }
