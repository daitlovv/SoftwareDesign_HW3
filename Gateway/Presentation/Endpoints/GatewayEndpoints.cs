...
public static class GatewayEndpoints
{
    ...
    {
        RouteGroupBuilder group = app.MapGroup("/works").WithTags("Работы");

        group.MapPost("/upload", async (HttpContext context, GatewayService service) =>
        {
            if (!context.Request.HasFormContentType)
            {
                return Results.BadRequest(new { error = "Требуется Content-Type: multipart/form-data" });
            }
            ...
            try
            {
                ...
                string? studentName = form["studentName"];
                string? workId = form["workId"];
        ...
        group.MapGet("/{workId}/reports", async (string workId, GatewayService service) =>
        {
            if (string.IsNullOrWhiteSpace(workId))
            {
                return Results.BadRequest(new { error = "Идентификатор работы обязателен" });
            ...
            try
            {
                IEnumerable<ReportDto> reports = await service.GetReportsAsync(workId);
